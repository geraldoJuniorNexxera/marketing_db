...
# Filtrar os dados para "GENERAL MOTORS DO BRASIL LTDA."
dados_gm = [item for item in dados_limpos if item.get("ds_razaosocial_comp") == "GENERAL MOTORS DO BRASIL LTDA."]

# Agrupar por "nu_nf" e selecionar a data mais recente de "dt_inclusao"
dados_gm_agrupados = {}
for item in dados_gm:
    nu_nf = item["nu_nf"]
    dt_inclusao = item["dt_inclusao"]
    if nu_nf not in dados_gm_agrupados or dt_inclusao > dados_gm_agrupados[nu_nf]["dt_inclusao"]:
        dados_gm_agrupados[nu_nf] = item

# Converter o dicionário de volta para uma lista
dados_gm_filtrados = list(dados_gm_agrupados.values())

# Combinar os dados filtrados com o restante dos dados
dados_finais = [item for item in dados_limpos if item.get("ds_razaosocial_comp") != "GENERAL MOTORS DO BRASIL LTDA."]
