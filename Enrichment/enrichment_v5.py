...
def consultar_bigquery(cnpjs):
    ...
    query = f"""
        ...
        SELECT 
            ...
            COALESCE(api.descricao_identificador_matriz_filial, '--') AS matriz_filial,
            COALESCE(api.descricao_situacao_cadastral, '--') AS situacao_cad,
            COALESCE(CAST(api.data_situacao_cadastral AS STRING), '--') AS data_sit_cad,
            COALESCE(api.descricao_motivo_situacao_cadastral, '--') AS motivo_sit_cad,
            COALESCE(CAST(api.cnae_fiscal AS STRING), '--') AS cnae_fiscal,
            COALESCE(api.cnae_fiscal_descricao, '--') AS cnae_descricao,
            ...
            COALESCE(n.soma_vl_documento, CAST(0 AS FLOAT64)) AS valor_disponivel_antecipacao,
            COALESCE(v.valor_bruto_2025, CAST(0 AS FLOAT64)) AS valor_pago_01_2025
        FROM cnpjs c
        LEFT JOIN `meta-vc-contabil.marketing_db.Meta_Contabil` mc
            ON c.cnpj = CAST(mc.cnpj AS STRING)
        LEFT JOIN `meta-vc-contabil.marketing_db_etl.base_cnae` api
